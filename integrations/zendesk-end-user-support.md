---
description: ZenDesk End User Support Portal integration — member-facing help center and ticket submission within nFinia Digital Banking.
---
...
{% hint style="info" %}
**Prerequisite:** A ZenDesk account with SSO (JWT or OAuth) must be provisioned for Diamond Credit Union. Contact the Tyfone implementation team for SSO token configuration.
{% endhint %}

| Parameter | Details |
| --- | --- |
| **Authentication** | JWT-based SSO using nFinia member session |
| **Subdomain** | `diamondcu.zendesk.com` |
| **Ticket Routing** | Routed to the `nfinia-support` view in the ZenDesk workspace |
| **Branding** | ZenDesk Help Center theme customizable to match Diamond CU brand |
