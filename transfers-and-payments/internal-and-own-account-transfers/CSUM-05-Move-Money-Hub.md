---
description: Move Money Hub
---

# Move Money Hub

> **Module:** Banking › Move Money

## Summary

The Move Money Hub is the central launchpad for all payment and transfer activities within nFinia Digital Banking. From one consolidated screen, members can access Quick Pay (loan payments), Internal Transfers, transfers to other Credit Union You, external ACH transfers, FedNow Instant Pay, Zelle, Bill Pay, Wire Transfers, and more.

This hub eliminates navigation complexity by surfacing every money-movement option in a clear tile layout. You identify the correct transfer type quickly and navigate directly to the appropriate workflow without searching through menus or sub-menus. The Move Money hub is also where Online Forms and Recipient Management are accessible.

**At a Glance**

<table><thead><tr><th width="254">Attribute</th><th>Detail</th></tr></thead><tbody><tr><td>Module</td><td>Move Money</td></tr><tr><td>Transfer Options</td><td>Quick Pay, Own Account, Other You, External, FedNow, Zelle, Bill Pay, Wire</td></tr><tr><td>Recipient Management</td><td>Accessible from Move Money hub</td></tr><tr><td>Online Forms</td><td>ACH, Wire, Address, Skip Pay forms accessible here</td></tr></tbody></table>

## Key Use Cases

| Use Case | Who Uses It | What They Do | Business Value |
| ------------------------ | ------------------------------ | ----------------------------------------------------------------- | -------------------------------------------------------------- |
| Select Transfer Type | Members initiating any payment | View all payment options on one screen, select the matching tile | Reduces payment errors from navigating to wrong workflow |
| Bill Pay Access | Members paying external bills | Click Pay Bills tile to navigate to bill payment service | One-click access to integrated bill pay from central hub |
| Wire Transfer Initiation | Members sending a wire | Click Wire Transfer tile to open the wire form | Direct access to domestic and international wire request forms |
| New Member Orientation | First-time digital banking you | Browse Move Money hub to understand all available payment options | Lowers learning curve — all options visible in one location |
## Step-by-Step Guide

**Step 1 — Navigate to Move Money Hub**

Start from the dashboard and navigate to Move Money. The Move Money Hub displays all payment and transfer options as tiles including Pay Bills, Quick Pay, Zelle Payment, Internal Transfers, Other Members, Same-Day Transfers, Send Instantly, Manage Recipients, Add Recipient, Transaction History, Scheduled Transfers, and P2P Transfer.

<figure><img src="../../.gitbook/assets/image (3) (1).png" alt=""><figcaption></figcaption></figure>
