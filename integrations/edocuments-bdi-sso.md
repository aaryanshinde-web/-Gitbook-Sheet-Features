...
| Access Level | Authenticated members (SSO token issued by nFinia post-login) |
...
| **Switch between multiple memberships** | Member with multiple account numbers | Uses the membership dropdown in BDI to toggle between different member numbers linked to their digital banking profile | Eliminates need for multiple logins; serves complex household or business structures |
...
| **Review enrollment and disclosures** | New member or compliance officer | Navigates to Enrollment or Disclosures tab within BDI portal | Ensures E-SIGN Act compliance; member can view the Online Statement Enrollment Disclosure and confirm paperless opt-in |
...
The BDI SSO integration is particularly important for credit unions with diverse membership profiles — households with both retail and business accounts, members with multiple share numbers, and members who require mortgage statements alongside standard monthly statements. By surfacing all document types under one authenticated session, Selfreliance FCU eliminates the need for members to maintain a separate BDI login.
...
- The member must have at least one statement available in BDI under their linked membership number(s).
- On mobile, the nFinia app must be installed and the member logged in.
...
The member logs into nFinia and clicks **Statements and Tax Forms** in the left sidebar (also accessible via **More** in the navigation menu). The breadcrumb path reads: `Banking > More > eStatements`.
...
No credentials are passed visibly to the member — the authentication is fully transparent.
...
The BDI portal opens at `uatestmt.businessdatainc.com/SelfrelianceFCU/ShowDocList.do`. The portal displays the member's name and a membership number dropdown pre-selected with their primary membership. The top navigation bar shows four tabs: **Documents**, **Enrollment**, **Disclosures**, and **Statement Search**.
...
If the member has multiple account numbers linked, they click the dropdown and select the desired membership. Each option is listed as `[MemberNumber] - [Name/Entity]`.
...
The member taps the hamburger menu (≡) in the nFinia mobile app to open the side navigation panel. The panel lists **Statements & Tax Forms** with the subtitle "View statements & Tax Forms."
...
**Single vs. multiple memberships:** If the member has only one membership number, the dropdown still renders but only shows one option. If multiple memberships exist, the member selects the desired one to load the correct document set.

**Read vs. unread documents:** A closed envelope icon indicates the document has not been opened before; an open envelope icon indicates the member has previously viewed it. This provides a visual delivery confirmation for each statement.
...
The member has successfully accessed their eDocuments when the BDI portal loads with their name, membership dropdown, and document list visible. On the nFinia side, the "This feature has been opened in another tab" banner (desktop) or the browser opening (mobile) confirms SSO was initiated successfully.
...
**SSO fails / session expired:** If the nFinia session has expired or the SSO token cannot be generated, the redirect will fail or display a BDI error page. The member should return to nFinia, log in again, and retry.
...
**Membership not found in BDI:** If the member's account number is not linked in BDI's system, the dropdown may not populate correctly. This is a back-office configuration issue requiring the FI's operations team to ensure the membership is enrolled in BDI's eStatements program.
...
The main landing screen of the BDI portal after SSO authentication. Displays the member's name, membership selector, and document type sections.
...
Opened when a member clicks or taps a specific statement date. Shows the full statement with per-account summaries and a PDF download option.
...
| Online Statement Enrollment Disclosure | Link | Opens the full text of the E-SIGN / paperless enrollment disclosure | Required for regulatory compliance with electronic document delivery; member can review the terms of their paperless enrollment |
